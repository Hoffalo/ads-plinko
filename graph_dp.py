    ...
    for peg_row, peg_column in peg_list:
        ...
        for child in (left_child, right_child):
            ...
            if isinstance(child, int):
                ...
                if slot_node not in neighbors_dictionary:
                    neighbors_dictionary[slot_node] = []
                neighbors_dictionary[peg_node].append((slot_node, 0.5))
            else:
                child_row, child_column = child
                child_peg_node = node_for_peg(child_row, child_column)
                if child_peg_node not in neighbors_dictionary:
                    neighbors_dictionary[child_peg_node] = []
                neighbors_dictionary[peg_node].append((child_peg_node, 0.5))

    number_of_columns = board_model.number_of_columns

    for column in range(number_of_columns):
        start_node = first_node_for_column(board_model, column)
        start_nodes_dictionary[column] = start_node
        if start_node is not None and start_node not in neighbors_dictionary:
            neighbors_dictionary[start_node] = []

    return neighbors_dictionary, start_nodes_dictionary
...
def compute_expected_values(board_model):
    neighbors_dictionary, start_nodes_dictionary = build_graph(board_model)
    expected_value_dictionary = {}

    def expected_value_for_node(node):
        if node in expected_value_dictionary:
            return expected_value_dictionary[node]
        ...
        if kind == "slot":
            ...
            expected_value_dictionary[node] = value
            return value

        neighbor_list = neighbors_dictionary.get(node, [])
        ...
        expected_value_dictionary[node] = total
        return total

    result_list = []
    for column in range(board_model.number_of_columns):
        start_node = start_nodes_dictionary.get(column)
